...
func init() {
	// загружаем данные из конфига в переменные окружения
	err := godotenv.Load()
	if err != nil {
...
func main() {
	...
	// логируем запросы (перед mux)
	handler := Logging(mux)

	// считываем порт из окружения
	port := os.Getenv("PORT")
	// формируем строку для подключения
	connStr := "localhost:" + port

	// запускаем сервер
	log.Println("Сервер запущен на:", connStr)
	err := http.ListenAndServe(connStr, handler)
	if err != nil {
